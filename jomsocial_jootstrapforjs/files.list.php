            ...
            <?php for($i=0;$i<=4;$i++){?>
				<?php if(!empty($data[$i])){?>
					<li id="file_<?php echo $data[$i]->id?>" >
						...
						<?php }?>
						<a <?php echo $data[$i]->link ?> class="jomNameTips" title="<?php echo JText::sprintf('COM_COMMUNITY_PHOTOS_UPLOADED_BY' , $data[$i]->user->getDisplayName() );?>" ><?php echo CStringHelper::truncate(strip_tags($data[$i]->name),40); ?></a>
