...
<div class="group">
			...
			<div class="app-box-content">
				<ul class="group-menus cResetList clrfix">
					<?php if( $config->get('creatediscussion') && ( ($isMember && !$isBanned) && !($waitingApproval) || $isSuperAdmin) ): ?>
					    <!-- Add Discussion -->
					    <li>
						    <a class="group-add-discussion" href="<?php echo CRoute::_('index.php?option=com_community&view=groups&task=adddiscussion&groupid=' . $group->id );?>">
					...
					<?php if( $allowCreateEvent && $config->get('group_events') && $config->get('enableevents') && ($config->get('createevents') || COwnerHelper::isCommunityAdmin()) ) { ?>
					    <li>
						    ...
						    <a class="group-create-event" href="<?php echo CRoute::_('index.php?option=com_community&view=events&task=import&groupid=' . $group->id);?>">
					...
					<?php } ?>
                                        ...
                                        <?php if( ($isAdmin) || ($isMine) || ($isMember && !$isBanned) ) { ?>
                                        <li>
                                            <a class="group-view-file" href="javascript:void(0)" onClick="joms.file.viewFile('group',<?php echo $group->id?>)"><?php echo JText::_('COM_COMMUNITY_FILES_VIEW_GROUP')?></a>
                                        </li>
                                        <?php } ?>
					...
					<div class="app-box-content">
						<div id="community-group-container">
							...
							<?php foreach( $videos as $video ) { ?>
								...
								<li class="cVideoItem jomNameTips" id="<?php echo "video-" . $video->getId() ?>" title="<?php echo $video->title; ?>">
    ...
    <div class="cMain clrfix">
        ...
        <!-- Group Photos -->
				...
				<div id="community-group-videos" class="app-box">
					...
					<div class="app-box-content">
						...
						<!--VIDEO ITEMS-->
						<div class="cVideoItems">
						
						<?php foreach( $videos as $video ) { ?>
						
							<!--VIDEO ITEM-->
							<div class="cVideoItem jomNameTips clrfix" id="<?php echo "video-" . $video->getId() ?>" title="<?php echo $video->title; ?>">
				
									<!--VIDEO THUMB-->
									<div class="cVideoThumb">
									<a class="cVideoThumbUrl" href="<?php echo $video->getURL(); ?>" style="width: <?php echo $videoThumbWidth; ?>px; height:<?php echo $videoThumbHeight; ?>px;">
									...
									<div class="cVideoSummary">
										<div class="cVideoTitle"><a href="<?php echo $video->getURL(); ?>"><?php echo $video->getTitle(); ?></a></div>									
										<div class="cVideoDetails small">
											<div class="cVideoHits"><?php echo JText::sprintf('COM_COMMUNITY_VIDEOS_HITS_COUNT', $video->getHits()) ?></div>
											<div class="cVideoLastUpdated"><?php echo JText::sprintf('COM_COMMUNITY_VIDEOS_LAST_UPDATED', $video->created ); ?></div>
        ...
        <?php } // if( $group->approvals == '0' || $isMine || $isMember || $isSuperAdmin ) ?>
		</div>
		<!-- end: .cMain -->
