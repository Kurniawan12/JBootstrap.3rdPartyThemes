...
<script type="text/javascript">
	...
	joms.jQuery('#createEvent').submit(function(event) {
		...
		<?php if ($event->id > 0 && $event->isRecurring()) { ?>
		...
		function toggleEventRepeat()
		{
			if( joms.jQuery('#repeat').val() != '' ){
				...
				if (joms.jQuery('#repeat').val() == 'daily') {
					limitdesc = '<?php echo sprintf(Jtext::_('COM_COMMUNITY_EVENTS_REPEAT_LIMIT_DESC'), COMMUNITY_EVENT_RECURRING_LIMIT_DAILY);?>';
				}else if (joms.jQuery('#repeat').val() == 'weekly') {
					limitdesc = '<?php echo sprintf(Jtext::_('COM_COMMUNITY_EVENTS_REPEAT_LIMIT_DESC'), COMMUNITY_EVENT_RECURRING_LIMIT_WEEKLY);?>';
				}else if (joms.jQuery('#repeat').val() == 'monthly') {
					limitdesc = '<?php echo sprintf(Jtext::_('COM_COMMUNITY_EVENTS_REPEAT_LIMIT_DESC'), COMMUNITY_EVENT_RECURRING_LIMIT_MONTHLY);?>';
				}
		...
		<td class="value">
			...
			<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_REPEAT_END_TIPS'); ?>">
				...
				<?php 
                                ...
                                if (!strtotime($event->repeatedend)) {
                                    $repeatend = null;
                                } else {
